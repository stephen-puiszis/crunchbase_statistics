...
# # # --------------------------------------- Individual Seeding --------------------------------- #
# HERSCHRIJVEN OOK VOOR COMP EN VCS
# individuals = Individual.offset(run_number * number_of_queries),limit(number_of_queries)
# # individuals.each_index do |j|

# #     person_data = JSON.parse(open(base_url+person+individuals[j].to_s+".js?"+api_key).read)
...
# #     fin_data = JSON.parse(open(base_url+fin+vcs[i].to_s+".js?"+api_key).read)
...
# #       company_data = JSON.parse(open(base_url+co+companies[i].to_s+".js?"+api_key).read)
